...
def load_data(file_path: Path) -> pd.DataFrame:
    """Load the cleaned reviews CSV and standardize column names."""
    try:
...
def extract_themes(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    for bank in df["bank"].unique():
        ...
        # Use 'themes' column from CSV
        themes = bank_df["themes"].value_counts().reset_index()
...
def identify_drivers_and_pain_points(df: pd.DataFrame) -> Dict[str, Dict[str, str]]:
    ...
    Drivers: top positive themes, Pain points: top negative themes.
    """
    result = {}
    for bank in df["bank"].unique():
        bank_df = df[df["bank"] == bank]
        positive_themes = bank_df[bank_df["sentiment_label"] == "positive"]["themes"].value_counts().head(2).index.tolist()
        negative_themes = bank_df[bank_df["sentiment_label"] == "negative"]["themes"].value_counts().head(2).index.tolist()
        result[bank] = {
...
def generate_recommendations(
    ...
    """Generate textual recommendations per bank."""
    recommendations = {}
